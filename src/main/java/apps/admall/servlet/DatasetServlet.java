...
public class DatasetServlet extends HttpServlet {
    ...
    private void handleGetAdmitConfig(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            try {
                String decrypted = CryptoHelper.decrypt(datasetIdStr);
                datasetId = Long.parseLong(decrypted);
            } catch (Exception decryptEx) {
                resp.setStatus(400);
                resp.getWriter().write("{\"error\":\"Invalid datasetId parameter format.\"}");
                return;
            }
        }
    ...
    private String escape(String s) {
        if (s == null) return "";
        return gson.toJson(s).substring(1, gson.toJson(s).length() - 1);
    }
}
