 ...
 * Queries the app_users table directly to fetch all mapped departments and links.
 */
@WebServlet("/api/department-info")
public class DepartmentInfoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            String sql = "SELECT userid, username, description, link " +
                         "FROM app_users " +
                         "WHERE userid = ? AND role = 'department' AND status = 'active'";
            ...
            if (infoArray.size() > 0) {
                resp.getWriter().write(gson.toJson(infoArray));
            } else {
                resp.setStatus(HttpServletResponse.SC_NOT_FOUND);
                resp.getWriter().write("{\"success\":false,\"message\":\"No department links configured for user.\"}");
            }
