...
public class RecordServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        // Extract and validate the candidate record ID parameter
        ...
        try {
            ...
            // It might be encrypted, try decrypting it
            try {
                String decrypted = CryptoHelper.decrypt(recordIdStr);
                recordId = Long.parseLong(decrypted);
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if ("department".equalsIgnoreCase(role)) {
                    ...
                    Set<Long> allowedDatasetIds = (Set<Long>) session.getAttribute("allowedDatasetIds");
                    @SuppressWarnings("unchecked")
                    Set<String> allowedDepts = (Set<String>) session.getAttribute("allowedDepts");

                    boolean dsAllowed = allowedDatasetIds != null && allowedDatasetIds.contains(datasetId);
                    ...
                    if (allowedDepts != null) {
                        for (String allowed : allowedDepts) {
                            if (matchDepartmentOrSchool(recordObj, allowed)) {
                                deptAllowed = true;
                                break;
                            }
                        }
                    }
                    if (!dsAllowed || !deptAllowed) {
                        resp.setStatus(HttpServletResponse.SC_FORBIDDEN);
                        resp.getWriter().write("{\"error\":\"Forbidden. Access denied for this record.\"}");
                        return;
                    }
                }
