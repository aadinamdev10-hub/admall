...
public class ReportServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (session != null) {
            ...
            if ("department".equalsIgnoreCase(role)) {
                ...
                Set<String> allowedDepts = (Set<String>) session.getAttribute("allowedDepts");
                String reqDept = req.getParameter("department");
                String reqDatasetId = req.getParameter("datasetId");

                boolean authorized = false;
                if (reqDept != null && !reqDept.trim().isEmpty() && allowedDepts != null) {
                    for (String allowed : allowedDepts) {
                        if (allowed.equalsIgnoreCase(reqDept.trim())) {
                            forcedDept = allowed;
                            authorized = true;
                            break;
                        }
                ...
                if (authorized) {
                    if (reqDatasetId != null && !reqDatasetId.trim().isEmpty()) {
                        try {
                            forcedDatasetId = Long.parseLong(reqDatasetId.trim());
        ...
        for (JsonObject record : records) {
            ...
            // Retrieve row values (supports multi-column checkbox choices)
            List<String> rowValues = getRowValues(record, resolvedRowField);
    ...
    private boolean isValidFieldName(String key) {
        if (key == null) return false;
        String lower = key.toLowerCase();
        if (lower.contains("declaration") || lower.contains("agree") || lower.contains(" rti ") || key.length() > 150) {
            return false;
        }
        return true;
    }

    private List<String> getRowValues(JsonObject record, String resolvedRowField) {
        List<String> list = new ArrayList<>();
        String val = getRecordValue(record, resolvedRowField);
        if (val != null && !val.trim().isEmpty() && !val.equalsIgnoreCase("Unknown")) {
            list.add(val.trim());
            return list;
        }
        
        // If not found or matched declaration/checkbox, try to scan for multi-column choices
        // Scan keys starting with A + digit + . (e.g. A1., A2., A3.) where value is "1", "Yes", or not empty/0
        for (String key : record.keySet()) {
            if (key.matches("^A\\d+\\..*")) {
                String cellVal = getRecordValue(record, key);
                if (cellVal != null && (cellVal.equals("1") || cellVal.equalsIgnoreCase("Yes") || cellVal.equalsIgnoreCase("True"))) {
                    // Extract the programme/department name by stripping the prefix (e.g., "A1. ")
                    String cleanedKey = stripPrefix(key);
                    list.add(cleanedKey);
                }
            }
        }
    ...
    private String resolveFieldName(JsonObject record, String fieldName) {
        ...
        if (record.has(fieldName) && isValidFieldName(fieldName)) return fieldName;
        
        // Match 2: Case-insensitive match
        for (String key : record.keySet()) {
            if (isValidFieldName(key) && key.equalsIgnoreCase(fieldName)) return key;
        }
        
        // Match 3: Clean alphanumeric containment match
        String cleanFieldName = clean(fieldName);
        for (String key : record.keySet()) {
            if (isValidFieldName(key) && clean(key).contains(cleanFieldName)) return key;
        }
        
        // Match 4: Substring case-insensitive match
        String lowerFieldName = fieldName.toLowerCase();
        for (String key : record.keySet()) {
            if (isValidFieldName(key) && key.toLowerCase().contains(lowerFieldName)) return key;
        }

        // Match 5: Strip prefix and do case-insensitive exact match
        String strippedFieldName = stripPrefix(fieldName);
        for (String key : record.keySet()) {
            if (isValidFieldName(key) && stripPrefix(key).equalsIgnoreCase(strippedFieldName)) return key;
        }
