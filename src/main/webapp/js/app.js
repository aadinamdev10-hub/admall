...
function handleRoleUIConstraints(session) {
  ...
  if (activeRole === "superadmin") {
    ...
    // Parse datasetId and department from the session links as fallback/enforcement
    let sessionDatasetId = 0;
    let sessionDept = "";
    let isAuthorized = false;

    // Support both new 'links' array and old 'link' string
    const allowedLinks = session.links || (activeLink ? [activeLink] : []);
    
    for (const link of allowedLinks) {
      if (!link) continue;
      const linkParams = parseLinkParams(link);
      const lDatasetId = Number(linkParams.datasetId);
      let lDept = linkParams.department;
      if (!lDept && link && !link.includes("=") && !link.includes("?")) {
        lDept = link;
      }
      
      // Check if this allowed link matches the requested department in URL (case-insensitive)
      if (urlDept && lDept && lDept.toLowerCase().trim() === urlDept.toLowerCase().trim()) {
        // If urlDatasetId is specified in URL, also match on datasetId
        if (urlDatasetId && lDatasetId && Number(lDatasetId) !== Number(urlDatasetId)) {
          continue;
        }
        sessionDatasetId = lDatasetId;
        sessionDept = lDept;
        isAuthorized = true;
        break;
      }
    ...
    // If no specific match was found but we don't have a urlDept parameter, default to the first allowed link
    if (!isAuthorized && !urlDept && allowedLinks.length > 0) {
      const firstLink = allowedLinks[0];
      const linkParams = parseLinkParams(firstLink);
      sessionDatasetId = Number(linkParams.datasetId);
      sessionDept = linkParams.department;
      if (!sessionDept && firstLink && !firstLink.includes("=") && !firstLink.includes("?")) {
        sessionDept = firstLink;
      }
      isAuthorized = true;
    }

    // Security check: If department user attempts to load an unauthorized department/dataset, redirect back to department.html
    if (urlDept && !isAuthorized) {
      console.warn("Unauthorized department access attempt:", urlDept);
      window.location.href = "department.html?v=" + new Date().getTime();
      return;
    }

    const forcedDatasetId = sessionDatasetId || Number(urlDatasetId);
    const forcedDept = sessionDept || urlDept || activeUsername;
    activeDeptFilter = forcedDept;
  ...
  fetch("../../api/session")
    ...
    .then(session => {
      ...
      if (activeRole === "department") {
        const isAllowedHtml = isIndexHtml || isAdmissionHtml;
        if (!isAllowedHtml || !urlParams.department) {
          window.location.href = "department.html?v=" + new Date().getTime();
          return;
        }
      ...
      handleRoleUIConstraints(session);
    })
    .catch(err => {
      console.error(err);
      window.location.href = "login.html";
...
function loadAllDrillDownRecords() {
  ...
  const drilldownUrl = activeDeptFilter 
    ? `../../api/drilldown?datasetId=${activeDatasetId}&department=${encodeURIComponent(activeDeptFilter)}` 
    : `../../api/drilldown?datasetId=${activeDatasetId}`;
...
function loadReport(datasetId, forcedDept) {
  ...
  const fetchUrl = forcedDept 
    ? `../../api/report?datasetId=${datasetId}&department=${encodeURIComponent(forcedDept)}` 
    : `../../api/report?datasetId=${datasetId}`;
...
function handleCellClick(rowField, rowValue, columnField, columnValue) {
  ...
  const drilldownUrl = activeDeptFilter 
    ? `../../api/drilldown?datasetId=${activeDatasetId}&rowField=${encodeURIComponent(rowField)}&rowValue=${encodeURIComponent(rowValue)}&columnField=${encodeURIComponent(columnField)}&columnValue=${encodeURIComponent(columnValue)}&department=${encodeURIComponent(activeDeptFilter)}` 
    : `../../api/drilldown?datasetId=${activeDatasetId}&rowField=${encodeURIComponent(rowField)}&rowValue=${encodeURIComponent(rowValue)}&columnField=${encodeURIComponent(columnField)}&columnValue=${encodeURIComponent(columnValue)}`;
        ...
        if (!Array.isArray(val)) {
          Object.entries(val).forEach(([secKey, fields]) => {
            items.push({
              type: "section_object",
              key: secKey,
              title: sectionTitles[secKey] || (secKey.length > 2 ? secKey : `Section ${secKey}`),
              fields: fields,
              sortKey: secKey
            });
          });
...
/**
 * Downloads the current filtered/displayed student list as a CSV file.
 */
function downloadStudentListCSV() {
  ...
  // Determine current active search filter from UI
  const searchInput = document.getElementById("drillDownSearchInput");
  const searchVal = searchInput ? searchInput.value.toLowerCase() : "";
  
  // Filter the list if a search query is active
  const recordsToExport = !searchVal.trim() 
    ? currentDrillDownRecords 
    : currentDrillDownRecords.filter(entry => {
        const record = entry.record;
        for (let key in record) {
          const val = String(record[key]).toLowerCase();
          if (val.includes(searchVal)) return true;
  ...
  if (recordsToExport.length === 0) {
    alert("No matching student records found to download.");
    return;
  }

  // Determine column headers and source mapping
  let columns = [];
  if (activeStudentListConfig && activeStudentListConfig.studentList && Array.isArray(activeStudentListConfig.studentList.columns)) {
    columns = activeStudentListConfig.studentList.columns;
  } else {
  ...
  // Helper functions matching table rendering logic
  function getValByPrefix(record, prefix) {
    if (!record || !prefix) return "";
    const lowerPrefix = prefix.toLowerCase().trim();
    for (let key in record) {
  ...
  recordsToExport.forEach(entry => {
    const rowValues = columns.map(col => {
      const val = resolveDrillDownCellValue(entry, col);
  ...
  // Create a descriptive file name using current dataset/filter if possible
  let filename = "student_list";
  const datasetSelect = document.getElementById("activeDatasetSelector");
  if (datasetSelect && datasetSelect.options[datasetSelect.selectedIndex]) {
    const datasetName = datasetSelect.options[datasetSelect.selectedIndex].text;
    if (datasetName && !datasetName.startsWith("--")) {
      filename += "_" + datasetName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    }
  }
  if (searchVal.trim()) {
    filename += "_filtered";
